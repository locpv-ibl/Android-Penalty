...
- With *stateful* authentication, a unique session id is generated when the user logs in. In subsequent requests, this session ID serves as a reference to the user details storead on the server. The session ID is *opaque*; it doesn't contain any user data.
